...
<div className="min-h-screen bg-[#F8FAFC] pt-24"> 
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-[1600px] mx-auto px-6 h-16 flex items-center justify-between">
          <Link to="/app" className="group flex items-center gap-2 text-slate-600 hover:text-indigo-600 transition-all font-semibold">
          ...
          <div className="flex items-center gap-3">
             ...
             <span className="text-slate-400 text-sm font-medium italic hidden sm:block">Auto-saving disabled</span>
             <button
                onClick={saveResume}
                disabled={loading}
              ...
              <div className="p-8">
                ...
                <div className="flex justify-between items-center mb-8 bg-slate-50 p-2 rounded-2xl">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      onClick={() => setActiveSectionIndex(prev => Math.min(prev + 1, sectuions.length - 1))}
                ...
                <div className="min-h-[450px]">
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    {activeSection?.id === "personal" && (
                      <Persnolinfoform data={resumeData.personal_info || {}} onChange={(data) => setResumeData(prev => ({ ...prev, personal_info: data }))} removrBackground={removeBackground} setremovrBackground={setRemoveBackground} />
                    )}
          ...
          <div className="lg:col-span-7">
            <div className="flex items-center justify-between mb-4 px-2">
                ...
                <div className="flex gap-2">
                    {resumeData.public && (
                      <button onClick={handleshare} className="flex items-center gap-1.5 text-xs font-bold text-emerald-600 bg-emerald-50 px-4 py-2 rounded-full hover:bg-emerald-100 transition-colors">
                        <Share2 size={14} /> Share
                      </button>
                    )}
                    <button onClick={changeResumeVisibill} className={`flex items-center gap-1.5 text-xs font-bold px-4 py-2 rounded-full transition-colors ${resumeData.public ? 'text-blue-600 bg-blue-50' : 'text-slate-500 bg-slate-100'}`}>
                      {resumeData.public ? <Eye size={14} /> : <EyeOff size={14} />}
                      {resumeData.public ? "Public" : "Private"}
                    </button>
                    <button onClick={downloadResume} className="flex items-center gap-1.5 text-xs font-bold text-white bg-slate-900 px-4 py-2 rounded-full hover:bg-black transition-all active:scale-95 shadow-lg shadow-slate-200">
